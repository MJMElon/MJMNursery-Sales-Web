...
export default function Auth() {
  ...
  async function loginWithPassword() {
    ...
    if (isSignUp) {
      ...
      const { data: signUpData, error } = await supabase.auth.signUp({ email: id, password, options: { data: { full_name: name } } });
      if (error) {
        if (error.message.toLowerCase().includes('database')) {
          const { data: retryData, error: retryErr } = await supabase.auth.signUp({ email: id, password });
          if (retryErr) { showStatus(retryErr.message, 'error'); return; }
          if (retryData && retryData.user) {
            await supabase.from('shared_profiles').upsert({ id: retryData.user.id, email: id, full_name: name, role: 'customer' }, { onConflict: 'id' });
          }
        } else { showStatus(error.message, 'error'); return; }
      } else if (signUpData && signUpData.user) {
        await supabase.from('shared_profiles').upsert({ id: signUpData.user.id, email: id, full_name: name, role: 'customer' }, { onConflict: 'id' });
      }
